...
type IUpdateCollaboratorModal = IUpdateModal<ICollaborator> & { collaborator_id: string };
...
export function UpdateCollaboratorModal({
  ...
  const onSubmit = useCallback(
    async ({ name, jobTitle, type, user }: ICollaboratorSchema) => {
      ...
      updateList(collaborator_id, data as ICollaborator);

      toast({ message: 'colaborador atualizado', severity: 'success' });

      closeModal();
    },
    [updateCollaborator, updateList, collaborator_id, toast, closeModal],
  );
