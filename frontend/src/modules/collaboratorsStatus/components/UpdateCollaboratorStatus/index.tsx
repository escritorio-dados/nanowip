...
} from '#modules/collaboratorsStatus/schema/collaboratorStatus.schema';

type IUpdateCollaboratorStatusModal = {
  openModal: boolean;
  closeModal: () => void;
  collaboratorStatus_id: string;
  handleUpdateData: (id: string, newData: ICollaboratorStatus) => void;
};
...
export function UpdateCollaboratorStatusModal({
  ...
  const onSubmit = useCallback(
    async ({ date, monthHours, salary, collaborator }: ICollaboratorStatusSchema) => {
      ...
      handleUpdateData(collaboratorStatus_id, data as ICollaboratorStatus);

      toast({ message: 'status do colaborador atualizado', severity: 'success' });

      closeModal();
    },
    [updateCollaboratorStatus, handleUpdateData, collaboratorStatus_id, toast, closeModal],
  );
