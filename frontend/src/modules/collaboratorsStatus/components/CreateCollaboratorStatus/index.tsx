...
type ICreateCollaboratorStatusModal = {
  openModal: boolean;
  closeModal: () => void;
  handleAdd(data: ICollaboratorStatus): void;
...
export function CreateCollaboratorStatusModal({
  ...
  const onSubmit = useCallback(
    async ({ date, monthHours, salary, collaborator }: ICollaboratorStatusSchema) => {
      ...
      handleAdd(data as ICollaboratorStatus);

      toast({ message: 'status do colaborador criado', severity: 'success' });

      closeModal();
    },
    [createCollaboratorStatus, handleAdd, toast, closeModal],
  );
    ...
    <>
      ...
      >
        <form onSubmit={handleSubmit(onSubmit)} noValidate>
          ...
          <CustomButton type="submit">Cadastrar Status do Colaborador</CustomButton>
        </form>
      </CustomDialog>
    </>
  );
