...
import { IDeliverableSchema, deliverableSchema } from '../../schemas/deliverable.schema';
import { IDeliverable, IUpdateDeliverableInput } from '../../types/IDeliverable';

type IUpdateDeliverableModal = Omit<IUpdateModal<IDeliverable>, 'updateList'> & {
  deliverable_id: string;
  section_id: string;
  updateList: (id: string, data: IDeliverable, section_id: string) => void;
};
...
export function UpdateDeliverableModal({
  closeModal,
  deliverable_id,
  section_id,
  openModal,
  updateList,
}: IUpdateDeliverableModal) {
  ...
  const onSubmit = useCallback(
    async ({ name, goal, progress, description, deadline }: IDeliverableSchema) => {
      const { error: updateErrors, data } = await deliverable({
        name,
      ...
      updateList(deliverable_id, data, section_id);

      toast({ message: 'entregável atualizado', severity: 'success' });

      closeModal();
    },
    [deliverable, updateList, deliverable_id, section_id, toast, closeModal, valueChains],
  );
