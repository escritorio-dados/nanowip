...
import { ICreateUser, IUser } from '#shared/types/backend/IUser';
...
import { createUserSchema, ICreateUserSchema } from '#modules/users/schema/create.user.schema';

import { ChoosePermissionArea } from './styles';

type ICreateUserModal = { openModal: boolean; closeModal(): void; handleAdd(user: IUser): void };
...
export function CreateUserModal({ closeModal, openModal, handleAdd }: ICreateUserModal) {
  ...
  const onSubmit = useCallback(
    async ({ permissions, ...rest }: ICreateUserSchema) => {
      ...
      handleAdd(newUser as IUser);

      closeModal();
    },
    [closeModal, createUser, handleAdd, toast],
  );
    ...
    <>
      ...
      >
        <form onSubmit={handleSubmit(onSubmit)} noValidate>
          ...
          <CustomButton type="submit">Cadastrar Usuario</CustomButton>
        </form>
      </CustomDialog>
    </>
  );
