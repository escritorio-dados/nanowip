...
type ICreateRoleModal = {
  openModal: boolean;
  closeModal: () => void;
  handleAdd(role: IRole): void;
};

export function CreateRoleModal({ openModal, closeModal, handleAdd }: ICreateRoleModal) {
  ...
  const onSubmit = useCallback(
    async ({ permissions, name }: IRoleSchema) => {
      ...
      handleAdd(newRole as IRole);

      toast({ message: 'papel criado', severity: 'success' });

      closeModal();
    },
    [closeModal, createRole, handleAdd, toast],
  );
    ...
    <>
      ...
      <CustomDialog open={openModal} closeModal={closeModal} title="Cadastrar papeis" maxWidth="md">
        <form onSubmit={handleSubmit(onSubmit)} noValidate>
          ...
          <CustomButton type="submit">Cadastrar Papel</CustomButton>
        </form>
      </CustomDialog>
    </>
  );
