...
import { IServiceProviderFilters } from '../../types/IServiceProvider';

export const defaultServiceProviderFilter: IServiceProviderFilters = {
  name: '',
  min_updated: null,
  max_updated: null,
};

export function ListServiceProvidersFilter({
  apiConfig,
  ...props
}: IListFilter<IServiceProviderFilters>) {
  const {
    handleSubmit,
    control,
    formState: { errors },
    reset: resetForm,
  } = useForm<IServiceProviderFilters>();

  return (
    <FilterListForm
      defaultFilter={defaultServiceProviderFilter}
