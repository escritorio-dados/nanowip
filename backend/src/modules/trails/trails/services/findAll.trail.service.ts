...
export class FindAllTrailService {
  ...
  async findAllLimited({ organization_id, query }: IFindAll<FindAllLimitedTrailsQuery>) {
  ...
  async findAllPagination({ organization_id, query }: IFindAll<FindAllPaginationTrailsQuery>) {
    const filters: IFilterValueAlias[] = [
      {
        field: 'name',
        values: [query.name],
