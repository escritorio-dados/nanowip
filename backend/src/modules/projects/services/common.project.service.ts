...
export class CommonProjectService {
  ...
  async validadeName({
    ...
    project_parent_id,
    organization_id,
  }: IFindByNameProjectDto): Promise<void> {
  ...
  async getProject({ id, organization_id, relations }: IGetProject): Promise<Project> {
    const project = await this.projectsRepository.findById(id, relations);

    await this.validateProject({ project, organization_id });
  ...
  async getParent({ id, organization_id, relations }: IGetProject): Promise<Project> {
    const project = await this.projectsRepository.findById(id, relations);
