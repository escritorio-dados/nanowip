...
import { FindOneCustomerService } from '@modules/customers/services/findOne.customer.service';
import { FindAllPortfolioService } from '@modules/portfolios/services/findAll.portfolio.service';
import { FindOneProjectTypeService } from '@modules/projects/projectTypes/services/findOne.projectType.service';

import { ProjectDto } from '../dtos/project.dto';
import { ProjectsRepository } from '../repositories/projects.repository';
import { ICreateProjectRepository } from '../repositories/types';
...
export class CreateProjectService {
  ...
  async execute({
    ...
    if (portfolios_id && portfolios_id.length > 0) {
      newProject.portfolios = await this.findAllPortfolioService.findAllByIds({
        portfolios_id,
        organization_id,
      });
    }
