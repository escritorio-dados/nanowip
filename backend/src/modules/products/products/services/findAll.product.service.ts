...
import { ISortConfig, ISortValue, sortSubs } from '@shared/utils/filter/configSortRepository';
...
type IFindAllLimited = IFindAll<FindAllLimitedProductsQuery> & { onlyRoot?: boolean };
...
export class FindAllProductService {
  ...
  async findAllPagination({ organization_id, query }: IFindAll<FindPaginationProductQuery>) {
    const filters: IFilterValueAlias[] = [
      {
        field: 'name',
        values: [query.name],
    ...
    const productSorted = sortSubs({
      list: products,
      order_by: query.order_by,
      sort,
      subfield: 'subproducts',
    });
    ...
    const productsWithStatus = productSorted.map(product => ({
      ...
      subproducts: product.subproducts.map(subproduct => ({
        ...selectFields(subproduct, ['id', 'name']),
        statusDate: getStatusDate(subproduct),
      })),
    }));

    const apiData: IResponsePagination<Product[]> = {
      pagination: {
        page: query.page,
        total_results,
  ...
  async findReport({ organization_id, query }: IFindAll<ReportProductQuery>) {
    const filters: IFilterValueAlias[] = [
      {
        field: 'name',
        values: [query.name],
